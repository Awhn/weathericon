...
```text
/city/South%20Korea/Seoul/Seoul/37.5665%2C126.978
       │
       ▼
도시 검색 → 날씨·대기질 조회 → 규칙 기반 판정 → 우산/마스크/복장 아이콘
```

- 진입 URI: `/city/{국가}/{지역}/{도시}/{위도,경도}`
- 루트 URI에서는 도시 입력 화면을 표시하고 선택이 끝나면 고유 지역 URI로 이동합니다.
...
`homepage`를 `.`으로 설정해 사용자/조직 페이지와 `/{repository}/` 형태의 프로젝트 페이지 모두에서 JS·CSS 경로가 저장소 하위 경로를 벗어나지 않게 했습니다. 고유 도시 URI를 직접 열어도 앱이 실행되도록 `build/index.html`을 `build/404.html`로 복제해 GitHub Pages의 SPA fallback으로 사용합니다.
